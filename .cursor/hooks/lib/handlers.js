...
export function handleBeforeSubmitPrompt(trace, input) {
  trace.update({
    name: (input.prompt && input.prompt.substring(0, 100)) || "User Prompt",
    input: input.prompt,
  });
  ...
  const generation = trace.generation({
    ...
    metadata: {
      ...
      attachment_count: (input.attachments && input.attachments.length) || 0,
      attachments: (input.attachments && input.attachments.map((a) => ({
        type: a.type,
        path: a.filePath,
...
export function handleAfterAgentResponse(trace, input) {
  const responseLength = (input.text && input.text.length) || 0;
  const lineCount = (input.text && input.text.split("\n").length) || 0;
  ...
  trace
    .span({
      ...
      metadata: {
        ...
        thinking_length: (input.text && input.text.length) || 0,
      },
    })
    .end();
...
export function handleBeforeShellExecution(trace, input) {
  trace
    .span({
      name: `Shell: ${(input.command && input.command.substring(0, 50)) || "command"}`,
      input: { command: input.command, cwd: input.cwd },
      metadata: {
        generation_id: input.generation_id,
        command_length: (input.command && input.command.length) || 0,
      },
    })
    .end();
...
export function handleAfterShellExecution(trace, input) {
  ...
  trace
    .span({
      name: `Shell Result: ${(input.command && input.command.substring(0, 40)) || "command"}`,
      ...
      metadata: {
        ...
        output_length: (input.output && input.output.length) || 0,
...
export function handleBeforeReadFile(trace, input) {
  ...
  trace
    .span({
      name: `Read: ${(input.file_path && input.file_path.split("/").pop()) || "file"}`,
...
export function handleAfterTabFileEdit(trace, input) {
  ...
  trace
    .span({
      ...
      output: {
        edit_count: editStats.editCount,
        edits: (input.edits && input.edits.map((e) => ({
          range: e.range,
          old_line: e.old_line,
          new_line: e.new_line,
        }))) || [],
